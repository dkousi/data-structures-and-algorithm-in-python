...
def find_flights(flight_time):
    ...
    for i in flight_details:
        a=i.split(":")
        if int(a[3])>=flight_time and int(a[3])<=flight_time+200:
            f.append(i)
    return f
...
def get_passenger_details(flight_detail):
    ...
    for i in flight_details:
        a=i.split(":")
        for k,j in passenger_details_dict.items():
            if str(a[0])==str(j[1]):
                res.append(j)
    return res            
    pass
...
def sort_passengers(passenger_pnr_list):
    #Remove pass and write your logic here
    passenger_pnr_list.sort(reverse=True)
...
def boarding(passenger_pnr_list):  
    #Remove pass and write your logic here
    f=[]
    for x in passenger_pnr_list:
        f.append(x)
    return f
    pass

def seating(passenger_queue):
    s=Stack(passenger_queue.get_max_size())
    for i in passenger_queue:
        s.push(passenger_queue.dequeue())
    return s
    #Remove pass and write your logic here
    pass
